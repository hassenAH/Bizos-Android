...
class ListUsersViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(UserProfileResponse: User) {
        photourl = UserProfileResponse.image
        roleUsercard!!.text = UserProfileResponse.role.toString()
        namecarduser!!.text = UserProfileResponse.first_name
        cardemailuser!!.text = UserProfileResponse.email
        phoneusercrd!!.text = UserProfileResponse.last_name
        levelusercard!!.text = UserProfileResponse.experience.toString()
        statususer!!.text = UserProfileResponse.experience.toString()
        createsatcard!!.text = parseDateToddMMyyyy(UserProfileResponse.experience.toString())
        if (UserProfileResponse.experience == 0) {
